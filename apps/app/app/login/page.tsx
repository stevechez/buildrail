...
/**
 * The one BuildRail login — every product (Field, Estimator, SiteVerdict,
 * Vault) redirects here instead of maintaining its own sign-in page (see
 * docs/platform/identity-foundation.md: "one primary service, one login").
 *
 * Magic-link only for now, deliberately auth-method agnostic: this page
 * and @buildrail/auth don't assume password auth anywhere, so adding
 * password or OAuth sign-in later is additive here, not a rework.
 *
 * `?returnTo=` carries the original product URL the user was trying to
 * reach (set by each product's middleware) so /auth/callback can send them
 * back to it — see that route for how it's threaded through.
 */
function LoginForm() {
  const params = useSearchParams();
